...
struct convection *cp;
{
	 ...
	 *
	 * 24Jul2022, Maiko (VE4KLM), this addition was a terrible idea in the
	 * way it was just plopped in. I should have checked for null pointers
	 * before calling the log function. It seems most glibc libraries will
	 * tolerate the passing of a NULL to Xprintf type functions, but still
	 * no excuse for this. Nobody's complained about crashes until 'now'.
	 *
	 * And even then, this might not be the reason for Jack (AA6HF) having
	 * an unstable system lately, so fixing this up at least rules it out.
	 *
	 * Also questioning if we even need to log this. It's just that I like
	 * to be aware of who is connecting and from where, for security, etc.
	 */
	if (pl)
	{
		if (*(pl->name))
		...
		{
			log (s, "convers session denied - no name");
			return;
		}
	}
	...
	/*
	 * 31Aug2016, Maiko (VE4KLM), give an indication in the log, never was
	 * 24Jul2022, Maiko (VE4KLM), check pointers before any logging !!!
	 */
	if (pl && *(pl->name))
