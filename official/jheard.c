...
static int jh_sizeof_table (void)
{
	...
	/* 30Dec2022, Maiko (VE4KLM), sometimes happens, JNOS will crash if iface misconfigured */
	if (cur_iface == NULLIF)
	{
		log (-1, "aprs interface misconfigured");
		return size;
	}
...
/*
 * 30Dec2022, Maiko (VE4KLM), this crashes JNOS if the iface does not exist,
 * we get a SIGSEGV in the jh_sizeof_table () function in this source file,
 * and what my point of setting cur_iface to the root of Ifaces back then.
 *
 * Just exit this function and do nothing to prevent any problems !
 *
	if (cur_iface == (struct iface*)0)
		cur_iface = Ifaces;
 */
	if ((cur_iface == NULLIF) || !(*ifacename))
	{
		log (-1, "aprs interface misconfigured");
