...
/* 17Feb2023, Maiko, Oops, need prototyping for icmpv6cksum or Ubuntu crashes */
#include "ipv6.h"
#include "icmpv6.h"

/*
 * 15Feb2023, Maiko (VE4KLM), IPV6 version of htontcp() further below,
 * was kind of reluctant to write this, but not sure how else to pass
 * a different checksum function specific to IPV6 into the flow, and
 * of course more code breakout as a result (which is not terrible).
 */
struct mbuf *htontcpv6 (register struct tcp *tcph, struct mbuf *data, struct ipv6 *ipv6)
{
    int16 hdrlen;
    struct mbuf *bp;
    register char *cp;
    ...
    *cp++ = 0;
    ...
    /* Recompute checksum, cleared earlier above - use icmpv6 one for now */
    // put16ub (&bp->data[16], icmpv6cksum (ipv6, bp, len_p (bp)));
    put16 (&bp->data[16], icmpv6cksum (ipv6, bp, len_p (bp)));
