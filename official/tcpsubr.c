...
register struct connection *conn;
{
	...
	int match;	/* 14Feb2023, Maiko */

    for(tcb=Tcbs;tcb != NULLTCB;tcblast = tcb,tcb = tcb->next)
	{
       	/* Yet another structure compatibility hack */
#ifdef	IPV6
		if (conn->ipver == IPVERSION)
		{
        	match = (conn->remote.port == tcb->conn.remote.port
		...
		{
#endif
			match = (conn->remotev6.port == tcb->conn.remotev6.port
					&& conn->localv6.port == tcb->conn.localv6.port
					&& !memcmp (conn->remotev6.address, tcb->conn.remotev6.address, 16)
					&& !memcmp (conn->localv6.address, tcb->conn.localv6.address, 16));
#ifdef	IPV6
		}
#endif
		if (match)
		{
/*
 * 21Feb2023, Maiko, Debugging, just comment out for now, telnet ipv6 from outside
 * is finally working now, lots left to do, but this is a HUGE milestone I think.
 *
			log (-1, "found tcb entry IPV4 port %d->%d IPV6 port %d->%d",
			tcb->conn.local.port, tcb->conn.remote.port,
			tcb->conn.localv6.port, tcb->conn.remotev6.port);

			log (-1, "matching conn = IPV4 port %d->%d IPV6 port %d->%d",
			conn->local.port, conn->remote.port,
			conn->localv6.port, conn->remotev6.port);
 */
...
struct connection *conn;
{
    ...
    // log (-1, "create_tcb ipver %d", tcb->conn.ipver);
