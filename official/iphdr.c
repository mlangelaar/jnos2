 ...
 *  (break out code that can be used for both ipv4 and ipv6, new function)
 */
#include "global.h"
#include "mbuf.h"
#include "ip.h"
...
/*
 * 09Feb2023, Maiko (VE4KLM), Decided to break out the mbuf chain portion
 * of the IP checksum code into it's own function, this to cut down on code
 * duplication. Both the ipv4 cksum() and my new icmpv6chksum() functions
 */

int16 cksum_mbuf_chain (struct mbuf *m, int32 *sum, int16 len)
{
    register int16 cnt, total, csum1;
    register int32 csum;
    register char *up;
    int swap = 0;
	...
	/* 09Feb2023, Maiko, 'sum' is a point now, passed */
        *sum += csum;
	...
	/* 09Feb2023, Maiko, Again, remember 'sum' is now a pointer */
...
int16 cksum (struct pseudo_header *ph, register struct mbuf *m, int16 len)
{
	...
	/*
	 * 09Feb2023, Maiko (VE4KLM), Breaking out common code
	 * also used by my the icmpv6cksum() function I wrote.
	 */
    return cksum_mbuf_chain (m, &sum, len);
}
