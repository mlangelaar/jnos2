...
#ifdef	IPV6
#include "ipv6.h"	/* 21Feb2023, Maiko, for singlembuser */
#endif
...
/* 21Feb2023, Maiko, Show IPV6 addresses now, would be nice
 * to use shortened form, but not quite there yet, also this
 * could be a bit cleaner looking code, good enough for now.
 */
...
void singlembuser (struct mbx *m, struct mbx *caller, FILE *tfile)
{
    char fsocket[
#ifdef	IPV6
	/* IPV6 sockaddr is 28, not 16 like most of the rest of them */
	sizeof(struct j2sockaddr_in6)
#else
	 ...
	 */
	int s, len = sizeof(struct j2sockaddr_in6);
#else
	...
	if (j2getpeername(m->user,fsocket,&len) == 0 && len)
	{
		...
		{
			up1 = itop(m->user);
			/* 01Apr2023, Maiko, Try out the shortform function */
			cp = ipv6shortform (human_readable_ipv6 (up1->cb.tcb->conn.remotev6.address), 0);
		}
		else
#endif
     ...
     */
      ...
      */
		...
		case AF_INET:
#ifdef	IPV6
		case AF_INET6:	/* 21Feb2023, Maiko (VE4KLM), we want to show IPV6 as well */
			if (*fsocket != 0x0a)
			{
#endif
			if((cp1 = strchr(cp,':')) != NULLCHAR)
