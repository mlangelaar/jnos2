...
/* #warning "default NO IAC processing in affect" */
	...
	/* 26Mar2023, Maiko, Allow forwarding with IPV6 systems */
		case '6':
			if (*argv[0] == '6')
			{
				/* 03Apr2023, Maiko, duplicate code moved to a new function */
				if (j2_ipv6_asc2ntwk (argv[1], sp.in6))
        	...
        	/* get the optional port number */
            	if(argc > 2)
                	sp.in6->sin6_port = atoip(argv[2]);
            	else
                	sp.in6->sin6_port = IPPORT_TELNET;

 	           if((m->user = j2socket(AF_INET6,SOCK_STREAM,0)) == -1) {
                	if(Mtrace)
                    	j2tputs("fwd: unable to open ipv6 telnet socket\n");
                	return -1;
            	}
