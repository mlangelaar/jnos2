...
#ifdef	IPV6
#include "ipv6.h"	/* 03Apr2023, Maiko */
#endif
...
#ifdef	IPV6
/* 27Mar2023, Maiko (VE4KLM), Should probably log these :] */
char *pinetv6 (struct j2socketv6 *s)
{
	static char buf[80], portinfo[10];

	/* 01Apr2023, Maiko, Try out the short form function */
	strcpy (buf, ipv6shortform (human_readable_ipv6 (s->address), 0));
	sprintf (portinfo, ":%d", s->port);
    strcat (buf, portinfo);
    return buf;
}
#endif
