...
#ifdef	IPV6
	/* 06Mar2023, Maiko, Time to try outgoing telnet sessions */
	if (!bbs && strchr (sp->name, ':'))
	{
...
/* 06Mar2023, Maiko, I am nowhere near ready to do IP6 DNS */

		if (strchr (sp->name, ':'))
		{
			/* taken from the ipv6addr stuff in ipv6cmd.c (for now) */

			/* 03Apr2023, Maiko, duplicate code moved into it's own function */
			if (j2_ipv6_asc2ntwk (sp->name, &fsocket6) != 1)
			{
