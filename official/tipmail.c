...
int telnet1 (int argc, char **argv, void *p)
{
    int16 port;
#ifdef	IPV6
	int ipv6 = 0;	/* 22Feb2023, Maiko, just a flag */
#endif
...
#ifdef	IPV6
	/*
	 * 22Feb2023, Maiko, give option so I can run both V6 and V4
	 */
	if (port == 65530) /* ha ha, cheating, uint16, neg # = 65530 :) */
	{
		ipv6 = 1;
	 ...
	 *
	 * 19Feb2023, Maiko, Temporary for testing !!!
	 *
	 * 22Feb2023, Maiko, Okay we can now start ipv6 and ipv4 listeners
	 */
#ifdef	IPV6
	if (ipv6)
    	return new_start_tcpv6 (port, "MBOX Server", mbx_incom, 8192, flags);
	else
#endif
