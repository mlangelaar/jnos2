...
int dombtelnet (int argc, char *argv[], void *p)
{
...
#ifdef	DONT_COMPILE
	log (m->user, "proc %s stk %ld siz %d auto %ld", Curproc->name,
		(long)(Curproc->stack), (int)(Curproc->stksize), (long)(&m));
#endif
	 ...
	 */
	copyipv6addr (resolve6 (argv[1]), fsocketv6.sin6_addr.s6_addr);

	if (fsocketv6.sin6_addr.s6_addr[0] != 0x00)		/* if resolve fails, goto ipv4 */
	{
...
#ifdef GWTRACE
    logmbox (m->user, m->name, "TELNET to %s:%d", argv[1], /* fsocket.sin_port */ port);
#endif
