			...
			if (ber_type == 6)
			{
				 ...
				 */
					/* Octet String */
						...
						odptr = systemResp (odptr, ber_ptr, &vblen, j2sysLocation ? j2sysLocation : "not defined" /* "my house" */);

					/* Octet String */
					else if (!memcmp (sysContact, ber_ptr, 8))
						odptr = systemResp (odptr, ber_ptr, &vblen, j2sysContact ? j2sysContact : "not defined" /* "phone 204 474 9273" */);
