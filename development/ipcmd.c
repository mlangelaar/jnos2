...
int doipheard(int argc,char *argv[],void *p)
{
    ...
    for(iph=Iph;iph;iph=iph->next)
	{
		...
		if (iph->addr6[0])
		{
        	...
        	if (tprintf ("%-32s", ipv6shortform (human_readable_ipv6 (iph->addr6), 0)) == EOF)
				return EOF;
		}
		else
		{
        ...
        if (tprintf("%-32s", inet_ntoa(iph->addr)) == EOF)
            return EOF;
#ifdef	IPV6
		}
#endif
 ...
 *
 * Make the function still works if IPV6 is NOT defined of course.
 */
