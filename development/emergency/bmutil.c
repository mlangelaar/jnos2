...
struct mbx *m;
{
    ...
    if ((m->privs&NO_LASTREAD) == 0) {
		...
		/* 02May2023, Maiko, We need better error reporting */
		log (m->user, "glr error unable to create usr file [%s]", buf);
                free(lognam);
                return;
            }
        }
...
struct mbx *m;
{
	...
	{
		/* 02May2023, Maiko, We need better error reporting */
		log (m->user, "slr error unable to rename usr file [%s] [%s]", buf, tmpname);
        /* Can't rename ??? */
        return;
	}
	...
	{
		/* 02May2023, Maiko, We need better error reporting */
		log (m->user, "slr error unable to create new usr file");

        /* can't creat new USR file ???*/
        if (rename(tmpname,buf))    /* try to undo the damage */
		{
			/* 02May2023, Maiko, We need better error reporting */
			log (m->user, "slr error unable to undo the damage");
		}
        return;
    }
	...
	{
		/* 02May2023, Maiko, We need better error reporting */
		log (m->user, "slr error unable to open rename filed");
    ...
    while(fgets(buf,sizeof(buf),tfile) != NULLCHAR) {
        ...
        if (fputs(buf,Alog) == EOF)
		{
			/* 02May2023, Maiko, We need better error reporting */
			log (m->user, "slr error fputs [%s] failed", buf);
			return;
		}
    }
    fclose(tfile);
