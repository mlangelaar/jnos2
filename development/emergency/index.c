 ...
 *
 * 02May2023, Maiko (VE4KLM), There is NO error checking going on in here
 * at all, and there are a lot of writes, so to keep the code minimal, a
 * new function is in order to check how much was written versus how much
 * has to be written - is this getting a bit carried away ? maybe, but I
 * need to make sure all writes and reads are in good order, right ?
 *
 * Note : this returns 1 on failure - less code for the caller ...
 */
int j2iwrite (int fd, void *buf, size_t count)
{
	ssize_t val = write (fd, buf, count);

	if (val != (ssize_t)count)
	{
		log (-1, "WriteIndex error val %ld len %ld errno %d",
			val, (long)count, errno);
...
int WriteIndex(int idx, struct mailindex *ind)
{
    ...
    if ((startoffset = lseek(idx,0,SEEK_CUR)) == -1L)
	{
		log (-1, "WriteIndex error startoffset %ld errno %d", startoffset, errno);
 		return -1;
	}
  
    /* write length bytes */
    if (j2iwrite(idx,&len,(size_t)sizeof(len)))
		return -1;
  
    /* write message id */
    if (j2iwrite(idx,&ind->msgid,(size_t)sizeof(ind->msgid)))
		return -1;
    xlen = sizeof(ind->msgid);
  
    /* write message type */
    if (j2iwrite(idx,&ind->type,(size_t)sizeof(ind->type)))
		return -1;
    xlen += sizeof(ind->type);
  
    /* write message status */
    if (j2iwrite(idx,&ind->status,(size_t)sizeof(ind->status)))
		return -1;
    xlen += sizeof(ind->status);
  
    /* write message size */
    if (j2iwrite(idx,&ind->size,(size_t)sizeof(ind->size)))
		return -1;
    ...
    /* write to-address */
    if(ind->to) {
        if (j2iwrite(idx,ind->to,(size_t)strlen(ind->to)))
			return -1;
        xlen += strlen(ind->to);
    }
    if (j2iwrite(idx,&null,(size_t)sizeof(null)))
		return -1;
    xlen += sizeof(null);
  
    /* write from-address */
    if(ind->from){
        if (j2iwrite(idx,ind->from,(size_t)strlen(ind->from)))
			return -1;
    ...
    /* write subject field */
    if(ind->subject){
        if (j2iwrite(idx,ind->subject,(size_t)strlen(ind->subject)))
			return -1;
    ...
    if(ind->replyto){
        if (j2iwrite(idx,ind->replyto,(size_t)strlen(ind->replyto)))
			return -1;
    ...
    if(ind->messageid){
        if (j2iwrite(idx,ind->messageid,(size_t)strlen(ind->messageid)))
			return -1;
    ...
    /* write received date */
    if (j2iwrite(idx,&ind->mydate,(size_t)sizeof(ind->mydate)))
		return -1;
    xlen += sizeof(ind->mydate);
  
    /* write date */
    if (j2iwrite(idx,&ind->date,(size_t)sizeof(ind->date)))
		return -1;
    xlen += sizeof(ind->date);
  
    /* write all Cc addressees */
    for(cc=ind->cclist;cc;cc=cc->next){
        if (j2iwrite(idx,cc->to,(size_t)strlen(cc->to)+1))
			return -1;
        xlen += strlen(cc->to)+1;
    }
    if (j2iwrite(idx,&null,(size_t)sizeof(null)))
    ...
    for(bbs=ind->bbslist;bbs;bbs = bbs->next){
        if (j2iwrite(idx,bbs->call,(size_t)strlen(bbs->call)+1))
			return -1;
    ...
    if (len != xlen) /* serious write or seek error */
 	{
		log (-1, "WriteIndex serious write or seek error len %d xlen %d truncate index", len, xlen);
        write(idx, &null, 0);    /* truncate index */
        return -1;
    }
        
    val = write(idx,&len,(size_t)sizeof(len));

    if (val != (ssize_t)sizeof(len))
	{
		log (-1, "WriteIndex (length) error val %ld len %d errno %d", val, len, errno);
	 	return -1;
	}
...
int write_index(char *name, struct mailindex *ind)
{
    ...
    if((idx=open(buf,READWRITEBINARY)) == -1)
	{
        ...
        if((idx=open(buf,CREATETRUNCATEBINARY,CREATEMODE)) != -1)
		{
            ...
            if (write_header(idx,&hdr) == -1)
			{
				log (-1, "write_index error creating index file");
				log (-1, "file [%s]", buf);
				 return -1;
			}
        }
		...
		{
			log (-1, "write_index error can't create index file");
				log (-1, "file [%s]", buf);
            return -1;
		}
    }
...
int write_header(int idx,struct indexhdr *hdr)
{
    ...
    val = write(idx,hdr,(size_t)sizeof(struct indexhdr));

	/* 02May2023, Maiko (VE4KLM), Need better indication of errors */
	if (val != (ssize_t)sizeof(struct indexhdr))
	{
		log (-1, "write_header error val %ld errno %d", val, errno);
        return -1;
	}
	return 0;
}
...
int read_header(int idx,struct indexhdr *hdr)
{
    ...
    if (val != (ssize_t)sizeof(struct indexhdr))
	{
		log (-1, "read_header error val %ld errno %d", val, errno);
        return -1;
	}
 	if (hdr->version != INDEXVERSION)
	{
		log (-1, "read_header bad index version");
        return -1;
	}
    return 0;
}
...
int read_index(int idx,struct mailindex *ind)
{
	...
	/* 02May2023, Maiko (VE4KLM), Need better indication of errors */
	ssize_t val = read(idx,&len,(size_t)sizeof(len));

    if (val != (ssize_t)sizeof(len))
	{
		log (-1, "read_index error val %ld errno %d", val, errno);
        return -1;
	}

	/* supposedly someone felt it wasn't reasonable to be over 8192 ? */
	if (len > 8192)
	{
		log (-1, "read_index error len [%d] is too big", len);
        return -1;
	}
	...
	/* 02May2023, Maiko (VE4KLM), Need better indication of errors */
    val = read(idx,buf,(size_t)len);
    if (val != (ssize_t)len)
	{
		log (-1, "read_index error short read val %ld len %d", val, len);
        free(buf);
        return -1;
    }
	...
	/* 02May2023, Maiko (VE4KLM), Need better indication of errors */
	errlen = (int)(++curr - buf);
    if (val != errlen) {  /* read garbage, no doubt */
		log (-1, "read_index error inconsistent [%d] len [%d]", val, len);
