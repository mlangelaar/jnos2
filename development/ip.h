...
struct raw_ip {
#ifdef	IPV6
	/* 08Apr2023, Maiko, Need to know whether IPV6 or IPV4 raw */
	char ipver;
#endif
...
struct iph {
    ...
    int32 addr;
#ifdef	IPV6
	/* 18Apr2023, Maiko, Adding IPV6 addresses, same listing */
...
#ifdef	IPV6
/* 08Apr2023, Maiko (VE4KLM), RAW socket support for IPV6 */
struct raw_ip *raw_ipv6 (int protocol, void (*r_upcall) (struct raw_ip*));
void del_ipv6 (struct raw_ip*);
#endif
...
/* In ipcmd.c */
/* 18Apr2023, Maiko (VE4KLM), Changed lead argument to void poitner
 * to support the inclusion of IPV6 in the ip heard list.
void log_ipheard __ARGS((int32 addr, struct iface *iface));
 */
void log_ipheard (void *ntwkaddr, struct iface *iface);
