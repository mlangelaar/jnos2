...
static int ri_error (int ref, struct mailindex *ind, char *errstr)
{
    log (-1, "read_index [%ld] [%s] (%d) : %s", ind->msgid, ind->messageid, ref, errstr);
...
int read_index(int idx,struct mailindex *ind) {
    int len, retval, save_errno;
    char *buf,*curr;
    struct cclist *newcc;
    struct fwdbbs *newbbs;

   /*
    * 30Dec2020, Maiko (VE4KLM), the error checking is terrible in here,
    * in the sense that you don't know WHY it happened, so let us try to
    * make a bit more information available in the logs to help us out.
    */  
    retval = read (idx, &len, (size_t)sizeof(len));

    if (retval == -1)
    {
        save_errno = errno;

        return ri_error (1, ind, strerror (save_errno));
    }

    if (!retval)
        return ri_error (1, ind, "end of file");

    /* Get the size of this index entry, ensuring it is reasonable */
    if (retval != (ssize_t)sizeof(len))
	return ri_error (1, ind, "short read");

    /* Get the size of this index entry, ensuring it is reasonable */
    if ((unsigned)len > 8192)
        return ri_error (1, ind, "larger then 8192");
  
    /* Now read the index */
    if((buf = mallocw((unsigned)len)) == NULL)
        return ri_error (1, ind, "no memory");
  
    retval = read (idx, buf, (size_t)len);

    if (retval == -1)
    {
        save_errno = errno;

        return ri_error (2, ind, strerror (save_errno));
    }

    if (!retval)
        return ri_error (2, ind, "end of file");

    /* Get the size of this index entry, ensuring it is reasonable */
    if (retval != (ssize_t)len)
    {
        free(buf);
	return ri_error (2, ind, "short read");
    }
	...
	return ri_error (3, ind, "final check indicates damaged indices");
    }
