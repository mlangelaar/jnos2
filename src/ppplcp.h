...
#define LCP_ENCRYPT     0x04	/* rfc1700 has this as Quality-Protocol ?? */
#define LCP_MAGIC       0x05
#define LCP_QUALITY     0x06    /* rfc1700 has this as RESERVED ?? */
#define LCP_PFC         0x07
...
#define LCP_N_ACFC      (1 << LCP_ACFC)
    ...
    int16 mru;          /* Maximum Receive Unit */
    int32 accm;         /* Async Control Char Map */
    int16 authentication;       /* Authentication protocol */
    int16 encryption;       /* Encryption protocol */
...
int doppp_lcp   __ARGS((int argc, char *argv[], void *p));
  
void ppp_ready  __ARGS((struct ppp_s *ppp_p));
  
void lcp_init   __ARGS((struct ppp_s *ppp_p));
