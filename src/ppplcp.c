...
static struct cmds DFAR Lcpside_cmds[] = {
    { "accm",     dolcp_accm, 0,  0,  NULLCHAR }, /* 05Jul2016, Maiko, compiler */
	...
	/* 11Oct2009, Maiko, use "%d" for int32 vars */
        ...
        if ( !x ) {
            ...
            if ( test ) {
                ...
                x = JnosClock << ((JnosClock & 0xf)+8);
		/* 05Jul2016, Maiko, Clock should be JnosClock */
            }
        }
        if ( x ) {
    ...
    switch(option_p->type) {
        ...
        /* Ensure that magic numbers are different */
            ...
            || lcp_p->remote.work.magic_number == lcp_p->local.work.magic_number) {
                side_p->work.magic_number += JnosClock;
		/* 05Jul2016, Maiko, Clock should be JnosClock */
                option_result = CONFIG_NAK;
            }
            break;
...
struct fsm_s *fsm_p;
{
    ...
    if ( lcp_p->local.want.negotiate & LCP_N_MAGIC ) {
        lcp_p->local.want.magic_number += JnosClock;
	/* 05Jul2016, Maiko, Clock should be JnosClock */
    }
